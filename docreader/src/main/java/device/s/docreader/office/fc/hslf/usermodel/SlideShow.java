package device.s.docreader.office.fc.hslf.usermodel;
...
import device.s.docreader.office.fc.hslf.exceptions.CorruptPowerPointFileException;
import device.s.docreader.office.fc.hslf.model.HeadersFooters;
import device.s.docreader.office.fc.hslf.model.Hyperlink;
import device.s.docreader.office.fc.hslf.model.MovieShape;
...
import device.s.docreader.office.fc.hslf.record.Document;
import device.s.docreader.office.fc.hslf.record.DocumentAtom;
import device.s.docreader.office.fc.hslf.record.ExAviMovie;
import device.s.docreader.office.fc.hslf.record.ExControl;
import device.s.docreader.office.fc.hslf.record.ExHyperlink;
import device.s.docreader.office.fc.hslf.record.ExHyperlinkAtom;
import device.s.docreader.office.fc.hslf.record.ExMCIMovie;
import device.s.docreader.office.fc.hslf.record.ExObjList;
import device.s.docreader.office.fc.hslf.record.ExObjListAtom;
import device.s.docreader.office.fc.hslf.record.ExOleObjAtom;
import device.s.docreader.office.fc.hslf.record.ExVideoContainer;
import device.s.docreader.office.fc.hslf.record.ExtendedParagraphHeaderAtom;
import device.s.docreader.office.fc.hslf.record.ExtendedPresRuleContainer;
import device.s.docreader.office.fc.hslf.record.ExtendedPresRuleContainer.ExtendedParaAtomsSet;
import device.s.docreader.office.fc.hslf.record.FontCollection;
import device.s.docreader.office.fc.hslf.record.HeadersFootersContainer;
import device.s.docreader.office.fc.hslf.record.PersistPtrHolder;
import device.s.docreader.office.fc.hslf.record.PositionDependentRecord;
...
import device.s.docreader.office.fc.hslf.record.RecordContainer;
import device.s.docreader.office.fc.hslf.record.RecordTypes;
import device.s.docreader.office.fc.hslf.record.SlideListWithText;
import device.s.docreader.office.fc.hslf.record.SlideListWithText.SlideAtomsSet;
import device.s.docreader.office.fc.hslf.record.SlidePersistAtom;
    ...
    private void buildSlidesAndNotes()
    {
        ...
        if (masterSLWT != null)
        {
            ...
            for (int i = 0; i < masterSets.length; i++)
            {
                ...
                if (r instanceof device.s.docreader.office.fc.hslf.record.Slide)
                {
                    TitleMaster master = new TitleMaster((device.s.docreader.office.fc.hslf.record.Slide)r, sheetNo);
                    master.setSlideShow(this);
                    tmr.add(master);
                }
                else if (r instanceof device.s.docreader.office.fc.hslf.record.MainMaster)
                {
                    SlideMaster master = new SlideMaster((device.s.docreader.office.fc.hslf.record.MainMaster)r, sheetNo);
                    master.setSlideShow(this);
                    mmr.add(master);
                }
            }
        ...
        // notesSLWT
        device.s.docreader.office.fc.hslf.record.Notes[] notesRecords;
        SlideAtomsSet[] notesSets = new SlideAtomsSet[0];
        Hashtable<Integer, Integer> slideIdToNotes = new Hashtable<Integer, Integer>();
        if (notesSLWT == null)
        {
            // None
            notesRecords = new device.s.docreader.office.fc.hslf.record.Notes[0];
        }
        else
        {
            // Match up the records and the SlideAtomSets
            notesSets = notesSLWT.getSlideAtomsSets();
            ArrayList<device.s.docreader.office.fc.hslf.record.Notes> notesRecordsL = new ArrayList<device.s.docreader.office.fc.hslf.record.Notes>();
            for (int i = 0; i < notesSets.length; i++)
            {
                ...
                if (r instanceof device.s.docreader.office.fc.hslf.record.Notes)
                {
                    device.s.docreader.office.fc.hslf.record.Notes notesRecord = (device.s.docreader.office.fc.hslf.record.Notes)r;
            ...
            notesRecords = new device.s.docreader.office.fc.hslf.record.Notes[notesRecordsL.size()];
            notesRecords = notesRecordsL.toArray(notesRecords);
        }

        // Now, do the same thing for our slides
        device.s.docreader.office.fc.hslf.record.Slide[] slidesRecords;
        SlideAtomsSet[] slidesSets = new SlideAtomsSet[0];
        if (slidesSLWT == null)
        {
            // None
            slidesRecords = new device.s.docreader.office.fc.hslf.record.Slide[0];
        }
        else
        {
            // Match up the records and the SlideAtomSets
            slidesSets = slidesSLWT.getSlideAtomsSets();
            slidesRecords = new device.s.docreader.office.fc.hslf.record.Slide[slidesSets.length];
            for (int i = 0; i < slidesSets.length; i++)
            {
                ...
                if (r instanceof device.s.docreader.office.fc.hslf.record.Slide)
                {
                    slidesRecords[i] = (device.s.docreader.office.fc.hslf.record.Slide)r;
                }
                /*else
                {
