 ...
 * 时间:               下午4:23:51
 */
package device.s.docreader.office.fc.ppt;
...
import device.s.docreader.office.common.bg.BackgroundAndFill;
import device.s.docreader.office.common.bg.Gradient;
import device.s.docreader.office.common.bg.LinearGradientShader;
import device.s.docreader.office.common.bg.RadialGradientShader;
import device.s.docreader.office.common.bg.TileShader;
import device.s.docreader.office.common.hyperlink.Hyperlink;
import device.s.docreader.office.common.pictureefftect.PictureEffectInfoFactory;
import device.s.docreader.office.common.shape.AbstractShape;
import device.s.docreader.office.common.shape.ArbitraryPolygonShape;
import device.s.docreader.office.common.shape.Arrow;
import device.s.docreader.office.common.shape.AutoShape;
import device.s.docreader.office.common.shape.GroupShape;
import device.s.docreader.office.common.shape.IShape;
import device.s.docreader.office.common.shape.LineShape;
import device.s.docreader.office.common.shape.PictureShape;
import device.s.docreader.office.common.shape.ShapeTypes;
import device.s.docreader.office.common.shape.TableCell;
import device.s.docreader.office.common.shape.TableShape;
import device.s.docreader.office.common.shape.TextBox;
import device.s.docreader.office.constant.AutoShapeConstant;
...
import device.s.docreader.office.fc.FCKit;
import device.s.docreader.office.fc.ShapeKit;
import device.s.docreader.office.fc.hslf.HSLFSlideShow;
import device.s.docreader.office.fc.hslf.model.Fill;
import device.s.docreader.office.fc.hslf.model.HeadersFooters;
import device.s.docreader.office.fc.hslf.model.Line;
import device.s.docreader.office.fc.hslf.model.MasterSheet;
import device.s.docreader.office.fc.hslf.model.Notes;
import device.s.docreader.office.fc.hslf.model.Shape;
import device.s.docreader.office.fc.hslf.model.ShapeGroup;
import device.s.docreader.office.fc.hslf.model.Sheet;
import device.s.docreader.office.fc.hslf.model.SimpleShape;
import device.s.docreader.office.fc.hslf.model.Slide;
import device.s.docreader.office.fc.hslf.model.SlideMaster;
import device.s.docreader.office.fc.hslf.model.TextShape;
import device.s.docreader.office.fc.hslf.model.TitleMaster;
import device.s.docreader.office.fc.hslf.record.BinaryTagDataBlob;
import device.s.docreader.office.fc.hslf.record.ClientVisualElementContainer;
import device.s.docreader.office.fc.hslf.record.DocumentAtom;
import device.s.docreader.office.fc.hslf.record.OEPlaceholderAtom;
import device.s.docreader.office.fc.hslf.record.PositionDependentRecordContainer;
import device.s.docreader.office.fc.hslf.record.Record;
import device.s.docreader.office.fc.hslf.record.SlideAtom;
import device.s.docreader.office.fc.hslf.record.SlideAtom.SSlideLayoutAtom;
import device.s.docreader.office.fc.hslf.record.SlideProgBinaryTagContainer;
import device.s.docreader.office.fc.hslf.record.SlideProgTagsContainer;
import device.s.docreader.office.fc.hslf.record.SlideShowSlideInfoAtom;
import device.s.docreader.office.fc.hslf.record.TimeAnimateBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeColorBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeCommandBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeEffectBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeMotionBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeNodeAttributeContainer;
import device.s.docreader.office.fc.hslf.record.TimeNodeContainer;
import device.s.docreader.office.fc.hslf.record.TimeRotationBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeScaleBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeSetBehaviorContainer;
import device.s.docreader.office.fc.hslf.record.TimeVariant;
import device.s.docreader.office.fc.hslf.record.VisualShapeAtom;
import device.s.docreader.office.fc.hslf.usermodel.PictureData;
import device.s.docreader.office.fc.hslf.usermodel.RichTextRun;
import device.s.docreader.office.fc.hslf.usermodel.SlideShow;
import device.s.docreader.office.fc.ppt.bulletnumber.BulletNumberManage;
import device.s.docreader.office.java.awt.Dimension;
import device.s.docreader.office.java.awt.Rectangle;
import device.s.docreader.office.java.awt.Rectanglef;
import device.s.docreader.office.java.awt.geom.Rectangle2D;
import device.s.docreader.office.pg.animate.ShapeAnimation;
import device.s.docreader.office.pg.model.PGModel;
import device.s.docreader.office.pg.model.PGNotes;
import device.s.docreader.office.pg.model.PGSlide;
import device.s.docreader.office.simpletext.font.FontTypefaceManage;
...
import device.s.docreader.office.ss.util.format.NumericFormatter;
import device.s.docreader.office.system.AbstractReader;
import device.s.docreader.office.system.BackReaderThread;
...
{   
    ...
    private device.s.docreader.office.common.borders.Line getShapeLine(SimpleShape shape)
    {
    	return getShapeLine(shape, false);
    }
    
    private device.s.docreader.office.common.borders.Line getShapeLine(SimpleShape shape, boolean isTableCellLine)
    {
        device.s.docreader.office.common.borders.Line line = null;
        if(shape != null && shape.hasLine())
        {
             ...
             device.s.docreader.office.java.awt.Color color = ((SimpleShape)shape).getLineColor();
             
             if(color != null)
             {
            	 line = new device.s.docreader.office.common.borders.Line();
    ...
    private BackgroundAndFill converFill(PGSlide pgSlide, Fill fill)
    {
        ...
        {        	
                    ...
                    || type == BackgroundAndFill.FILL_SHADE_SHAPE)
            {
            	...
            	device.s.docreader.office.java.awt.Color fillColor = fill.getForegroundColor();
            	device.s.docreader.office.java.awt.Color fillbackColor = fill.getFillbackColor();
    ...
    private void processNotes(PGSlide pgSlide, Notes notes)
    {
        if (notes != null)
        {
            ...
            for (Shape shape : notes.getShapes())
            {
                ...
                if(shape instanceof device.s.docreader.office.fc.hslf.model.AutoShape  // 文本框
                    || shape instanceof device.s.docreader.office.fc.hslf.model.TextBox)  // 占位符
                {
...
//        }
        ...
        if (shape instanceof Line
            || shape instanceof device.s.docreader.office.fc.hslf.model.Freeform
            || shape instanceof device.s.docreader.office.fc.hslf.model.AutoShape
            || shape instanceof device.s.docreader.office.fc.hslf.model.TextBox
            || shape instanceof device.s.docreader.office.fc.hslf.model.Picture)
        {  
            if (shape instanceof Line)
            {
                if (line != null)
            ...
            else if(shape instanceof device.s.docreader.office.fc.hslf.model.Freeform)
            {              
                if (fill != null || line != null)
                {
                    ...
                    if (startArrowType > 0)
                    {
                    	ArrowPathAndTail arrowPathAndTail = ((device.s.docreader.office.fc.hslf.model.Freeform)shape).getStartArrowPathAndTail(rect);
                    ...
                    if (endArrowType > 0)
                    {                        
                    	ArrowPathAndTail arrowPathAndTail = ((device.s.docreader.office.fc.hslf.model.Freeform)shape).getEndArrowPathAndTail(rect);
                    	if(arrowPathAndTail != null && arrowPathAndTail.getArrowPath() != null)
                        {
                    ...
                    Path[] paths = ((device.s.docreader.office.fc.hslf.model.Freeform)shape).getFreeformPath(rect,
            ...
            else if(shape instanceof device.s.docreader.office.fc.hslf.model.AutoShape
                    || shape instanceof device.s.docreader.office.fc.hslf.model.TextBox)
            {
                // autoShape
            ...
            // 图片
            else if(shape instanceof device.s.docreader.office.fc.hslf.model.Picture)
            {   
                device.s.docreader.office.fc.hslf.model.Picture poiPic = (device.s.docreader.office.fc.hslf.model.Picture)shape;
        ...
        else if(shape instanceof device.s.docreader.office.fc.hslf.model.Table)
        {
            device.s.docreader.office.fc.hslf.model.Table poiTable = (device.s.docreader.office.fc.hslf.model.Table)shape;
            if (poiTable != null)
            {
     ...
     * @param pgSlide
     */
    private void processTable(PGSlide pgSlide, device.s.docreader.office.fc.hslf.model.Table poiTable, GroupShape parent, int slideType)
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < columns; j++)
            {
                ...
                device.s.docreader.office.fc.hslf.model.TableCell poiCell = poiTable.getCell(i, j);
                if (poiCell != null)
                {
        ...
        {
        	device.s.docreader.office.common.borders.Line aLine = getShapeLine(line, true);
            if (aLine != null)
            {
    ...
    private int getBorderColor(Line line)
    {
        if (line != null)
        {
            device.s.docreader.office.java.awt.Color color = line.getLineColor();
            if (color != null)
            {
    ...
    private void processNormalTextShape(TextBox tb, TextShape ts, Rectangle rect, int slideType, int placeHolderID)
    {
        ...
        if (text != null && text.trim().length() > 0)
        {
            ...
            int len = text.length();
            device.s.docreader.office.fc.hslf.model.Hyperlink[] links = ts.getTextRun().getHyperlinks();
            int start = 0;
            // title type just a paragraph needs be processed specially
            if (ts.getTextRun().getRunType() != device.s.docreader.office.fc.hslf.record.TextHeaderAtom.TITLE_TYPE)
            {
                for (int i = 0; i < len; i++)
                {
                    if (abortReader)
    ...
    private void processParagraph(SectionElement secElem, TextShape ts, String text,
        device.s.docreader.office.fc.hslf.model.Hyperlink[] links, int start, int end, int placeHolderID)
    {
        ...
        device.s.docreader.office.fc.hslf.record.TextRulerAtom ruler = ts.getTextRun().getTextRuler();
        if (ruler != null)
        {
...
//      }
     ...
     * 
     */
    private int converterColor(device.s.docreader.office.java.awt.Color color)
    {
//        if(color.getAlpha() == 0)
//        {
